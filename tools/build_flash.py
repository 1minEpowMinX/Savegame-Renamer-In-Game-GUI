"""Compile flash/renamer.as into the .gfx the mod ships.

The container is generated by make_swf.py rather than taken from another mod, so
the only binary in the source tree is the build product. FFDec compiles the
ActionScript: it has no "compile one file" mode, so the empty script is exported
to a scratch folder, overwritten with renamer.as, and imported back.
...
def build():
    ...
    with open(container, "wb") as f:
        f.write(make_swf.build(STAGE_WIDTH, STAGE_HEIGHT))
    ...
    if not os.path.isfile(target):
        raise RuntimeError("FFDec exported no frame script; the container is malformed")
